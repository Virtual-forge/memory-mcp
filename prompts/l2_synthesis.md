...
contains `scope`, `new_atoms`, `scene_names`, and existing rows grouped by
category. Use only the supplied atoms as new evidence. Existing rows are
context for matching and updating, not new evidence.
...
Confidence 0.8 and above is direct, unambiguous, confirmed, or applied. These
atoms may change the settled content of a summary. Atoms below 0.8 are proposed
or not confirmed; link them for provenance, but do not rewrite settled summary
content as though they were facts. Surface them as a distinct "Proposed, not yet
applied" note instead. When a destination contains only low-confidence atoms,
preserve the existing summary if one exists and use the proposed note as the
new row's content when no settled summary exists.

Re-render the complete markdown `summary` for every destination that receives
new atoms. Keep it concise and factual. Never paste raw transcripts. For a
`tasks` destination, use one of `todo`, `doing`, `done`, `blocked`, or
`cancelled` for `status`.
