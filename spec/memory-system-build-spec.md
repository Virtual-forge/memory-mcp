...
Directory names below match the spec's own section numbers in comments so
the two stay traceable to each other — when a section changes, the file
it maps to should be obvious without re-deriving it.

```
memory-manager/
├── prompts/
│   ├── extraction_judge.md      # §2.2 — written
│   ├── l2_synthesis.md          # §2.3 — written
│   └── reranker.md              # §3.5 — written
├── memory_manager/
│   ├── config.py                # env vars: DB URL, Qdrant URL, embedding + LLM provider config
│   ├── db/
│   │   ├── schema.sql           # source of truth for §1's tables
│   │   └── connection.py
│   ├── models/                  # thin data classes matching §1 — not ORM-heavy
│   │   ├── turn.py              # §1.1
│   │   ├── atom.py              # §1.2
│   │   ├── entity.py            # §1.2's entities registry
│   │   └── l2_row.py            # one shape, reused by every §1.4 table
│   ├── embedding.py             # dense + sparse embed calls — shared by indexing (write) and retrieval (read), not duplicated in each
│   ├── ingestion/
│   │   └── turns.py             # write_turn() — §1.1, never blocks on the judge
│   ├── extraction/              # §2.1-2.2, L0 → L1
│   │   ├── triggers.py          # inactivity + volume-cap, §2.1
│   │   ├── judge.py             # calls prompts/extraction_judge.md
│   │   ├── entity_resolution.py # match-before-create, §1.2
│   │   └── idempotency.py       # atom_sources coverage check, §2.4
│   ├── synthesis/                # §2.3, L1 → L2
│   │   ├── routing.py            # category routing
│   │   ├── upsert.py             # upsert-vs-new-row matching
│   │   └── judge.py              # calls prompts/l2_synthesis.md
│   ├── indexing/
│   │   └── qdrant_writer.py      # upserts atoms + L2 rows into the one shared collection, §1.4
│   ├── retrieval/                # §3
│   │   ├── recall.py             # thin orchestrator ONLY — see §0.1's warning about this function specifically
│   │   ├── expand_sources.py     # types → sources, §1.5 / §3.1
│   │   ├── modes/
│   │   │   ├── vector.py         # §3.2
│   │   │   ├── bm25.py           # §3.3
│   │   │   └── hybrid.py         # §3.4
│   │   ├── rerank.py             # §3.5
│   │   └── candidates.py         # §3.6 search_candidates + fetch
│   └── mcp/
│       ├── server.py
│       └── tools.py              # remember, recall, search_candidates, fetch, update, forget — §4.1
├── tests/
│   ├── eval/
│   │   └── extraction_eval_set.jsonl  # hand-labeled real transcripts — §5 step 2 requires this before trusting the judge
│   ├── test_extraction.py
│   ├── test_synthesis.py
│   ├── test_retrieval.py
│   └── test_mcp_tools.py
...
All three prompts referenced by this spec now exist: `extraction_judge.md`
(§2.2), `l2_synthesis.md` (§2.3), and `reranker.md` (§3.5, added below the
project tree above since reranking wasn't in the original layout).
...
**Drill-down is a two-hop join for L2, one-hop for L1 — not a direct
edge.** By design there's no L2 → L0 shortcut (§1.4 links L2 only to its
immediate parent atoms). The exact turns behind any record are still
fully recoverable:

```sql
-- turns behind a single atom (one hop)
select t.* from atom_sources asrc
join turns t on t.id = asrc.turn_id
where asrc.atom_id = :atom_id;

-- turns behind an L2 row (two hops, via that category's junction table)
select distinct t.* from project_atoms pa
join atom_sources asrc on asrc.atom_id = pa.atom_id
join turns t on t.id = asrc.turn_id
where pa.project_id = :id;
```

Now that retrieval defaults to L2-first (§3.1), this is a runtime
capability the calling agent will actually use — not just an audit
query — so expose it as its own MCP tool (§4.1) rather than leaving it as
something only reachable via raw SQL.
...
- Re-render `summary` for any row that received new atoms, and link the
  new atoms via that category's `*_atoms` junction table. **Gate what's
  allowed to change the settled parts of the summary by confidence** — an
  atom below ~0.8 confidence (§1.2) represents something discussed or
  proposed, not confirmed or applied, and must not silently rewrite the
  summary as if it were settled fact. Link it for provenance, but surface
  it as a distinct "proposed, not yet applied" note rather than merging
  it into the row's core description. Only high-confidence atoms — the
  ones the extraction judge already marked as direct and unambiguous, not
  hedged — get to change what the summary asserts as true. This is not a
  new mechanism: the extraction judge's attribution rule (§2.2) already
  produces this signal at write time; this is synthesis actually using it.
...
name (`projects`, `tools`, `skills`, `profiles`, `users`, ...). **Default
when omitted is every L2 category, excluding `atoms`** — L2 content is
synthesized, lower-noise, and what most queries should be answered from
first. `atoms` is opt-in, not part of the default blend. This requires
all atoms and all L2 rows to live in **one shared Qdrant collection**,
not one collection per table, with each point's `source_table` payload
(§1.4) used as a pre-filter on the query. Do not use per-category
collections — that turns a multi-source query into multiple round trips
plus client-side merging for no benefit, when a single filtered query
does the same thing natively.

**Recall escalation is bounded and staged by the AgentOS dispatcher.** The
low-level `recall()` API does not retry internally and still defaults to
L2 categories. The example AgentOS dispatcher gives each run at most three
memory lookup attempts: the first two search L2 only, and the third searches
L1 atoms together with L2. A fourth attempt is refused. The dispatcher uses
the calling agent's judgment for whether a result is empty, partial, or does
not address the question, but the attempt count and source set are enforced
by code. Direct MCP clients may still choose their own explicit `sources`.

After a result has been returned, an explicit request for exact wording,
evidence, rationale, history, or other extra detail may use `drill_down(id,
source_table)` to retrieve its supporting L0 turns. This is provenance lookup,
not a new memory search, and it must use an ID and source table from a prior
result.
...
# an empty result (no types, no sources) means "every L2 category" —
# atoms are never implied by an empty/omitted request.
```
...
natural defaults — whether from an explicit multi-category call, from
expanding a mixed type like `workspace` (`projects` + `docs`, §1.5), or
from the plain default of "every L2 category" (which spans all of them) —
fall back to `hybrid` rather than picking one table's default arbitrarily
for all of them. In practice this means an unscoped `recall(query, scope)`
call almost always lands on `hybrid` — that's the correct default for a
broad first search, not a bug. An explicit `mode` from the caller always
overrides this.
...
pool using `prompts/reranker.md` (a lightweight LLM scoring call — a small
dedicated reranker model is the alternative if you stand one up), producing
a calibrated relevance score per candidate. Apply `min_score` against
*that* score, not the raw retrieval score. Return whatever clears the bar
— a variable count, never a fixed N padded with weak matches.

Candidates handed to the reranker must be normalized to `{id, title, content}`
before the call — `atoms` have no `title` (§1.2, by design: a `statement` is
already minimal), so pass `title: null` for those rather than omitting the
field or forcing something into it.
...
- `recall(query, scope, mode, sources, top_k, min_score)` — wraps
  §3.1-3.5 directly. Defaults to L2 categories only (§3.1). The example
  AgentOS dispatcher applies the three-attempt L2, L2, then L1+L2 policy
  and refuses further memory searches for that run.
...
- `drill_down(id, source_table)` — wraps §1.3's provenance join; returns
  the exact L0 turns behind an atom or an L2 row. With retrieval
  defaulting to L2-first, this is how a caller gets from a synthesized
  summary down to what was actually said when the user explicitly asks for
  extra detail or supporting evidence.
...
### 4.3 Project documentation imports

Project documentation is opt-in and starts with deterministic identity
selection. `list_projects(scope)` returns every registered project in stable
name/UUID order; with no scope it searches all project scopes. The caller must
present those projects through Agno's native structured
`UserFeedbackTools.ask_user` and use the confirmed UUID for the rest of the run.
Because that selector supports at most three projects plus an Other options option,
larger lists must fail clearly and be narrowed with an exact scope rather than
being truncated. The Other options option opens a second structured feedback
question with Recall memory and Cancel. The former uses generic recall with the
original request; the latter stops the lookup. A real project selection continues
to project recall with the confirmed UUID.
`resolve_project(name, scope)` remains available for explicit exact, alias, and
partial name lookup workflows. Identity confirmation is separate from the
recall-attempt budget.

The importer accepts a directory tree containing one or more UTF-8 Markdown
files. Only `.md` and `.markdown` files are supported. Discovery is recursive,
stable, and symlink-free; common build, cache, vendor, and virtual-environment
directories are ignored. Each file is versioned by content hash, split at
heading and paragraph boundaries, and persisted as a `project_documents` row,
`project_document_chunks` rows, and provenance-rich L0 turns. Relative paths,
headings, and line ranges are retained because documentation subdirectories
often represent the topic they document.

Project chunks use a dedicated L1 judge. Unlike ordinary conversational
extraction, one authoritative Markdown chunk may support a project atom. Those
atoms are synthesized only into project-scoped `project_sections`; they must
not enter generic L2 rows. Generic extraction, synthesis, and retrieval use
`project_id is null` semantics, while project retrieval requires the confirmed
UUID and filters both Postgres and Qdrant by it.

An import is idempotent for unchanged files. A changed or deleted file
deactivates all of its old chunks before stale-source evaluation. Atoms with no
remaining active project-document source are hidden, affected project sections
are rebuilt from surviving atoms, and obsolete atom or section points are
removed from Qdrant. The worker exposes this flow as:

```text
memory-worker import-project <folder> --project-id <confirmed-uuid>
```
